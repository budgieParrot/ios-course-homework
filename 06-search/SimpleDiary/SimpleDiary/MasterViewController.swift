...
class MasterViewController: SearchResultsViewController, UISearchResultsUpdating,
    ...
    private func recordsWithNameStarting(namePrefix: String) -> [DiaryRecord] {
        let array = objects.filter() { self.hasCaseInsensitivePrefix($0.name!, namePrefix) }
        return array
    }
    
    private func hasCaseInsensitivePrefix(lhs: String, _ rhs: String) -> Bool {
        if let _ = lhs.rangeOfString(rhs, options: [.CaseInsensitiveSearch, .AnchoredSearch], range: nil, locale: nil) {
            return true
        }
        return false
    }
    
    func displayIndexPathForPerson(record: DiaryRecord) -> NSIndexPath? {
        let sectionsArray: [String] = sortedObjects?.allKeys as! [String]
        for section in sectionsArray {
            if let recordsArray = sortedObjects?.valueForKey(section) as? [DiaryRecord] {
                if let row = recordsArray.indexOf(record) {
                    return NSIndexPath(forRow: row, inSection: getSectionForHeader(section))
                }
            }
        }
        return nil
    ...
    override func tableView(tableView: UITableView, didSelectRowAtIndexPath indexPath: NSIndexPath) {
        if let resultsController = resultsTableController
            where tableView == resultsController.tableView {
                let record = resultsController.records[indexPath.row]
                if let indexPath = displayIndexPathForPerson(record) {
    ...
    private func getSectionForHeader(header: String) -> Int {
        switch(header) {
        case TODAY:
            if(sortedObjects?.objectForKey(TODAY) != nil) {
                return 0
        ...
        case THIS_WEEK:
            if(sortedObjects?.objectForKey(THIS_WEEK) != nil && sortedObjects?.objectForKey(TODAY) != nil) {
                return 1
            } else {
                if(sortedObjects?.objectForKey(THIS_WEEK) != nil) {
                    return 0
        ...
        case THIS_MONTH:
            if(sortedObjects?.objectForKey(THIS_MONTH) != nil && sortedObjects?.objectForKey(THIS_WEEK) != nil
                && sortedObjects?.objectForKey(TODAY) != nil) {
                    return 2
            } else {
                if(sortedObjects?.objectForKey(THIS_MONTH) != nil && sortedObjects?.objectForKey(THIS_WEEK) != nil) {
                    return 1
        ...
        case EARLIER:
            if(sortedObjects?.objectForKey(EARLIER) != nil && sortedObjects?.objectForKey(THIS_MONTH) != nil && sortedObjects?.objectForKey(THIS_WEEK) != nil
                && sortedObjects?.objectForKey(TODAY) != nil) {
                    return 3
            } else {
                if(sortedObjects?.objectForKey(EARLIER) != nil && sortedObjects?.objectForKey(THIS_MONTH) != nil && sortedObjects?.objectForKey(THIS_WEEK) != nil) {
                    ...
                    if(sortedObjects?.objectForKey(EARLIER) != nil && sortedObjects?.objectForKey(THIS_MONTH) != nil) {
                        return 1
                    } else {
                        if(sortedObjects?.objectForKey(EARLIER) != nil) {
                            return 0
                        }
                    }
                }
    ...
    func updateSearchResultsForSearchController(searchController: UISearchController) {
        ...
        resultsTableController?.records = searchResults
    }
